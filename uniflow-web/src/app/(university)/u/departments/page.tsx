          ...
          <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
            <div style={{ position: "relative", flex: 1 }}>
              <select
                ...
                onChange={(e) => {
                  if (e.target.value) {
                    onAssignHod(dept.id, e.target.value);
                    setAssigning(false);
                  }
                }}
              >
...
export default function DepartmentsPage() {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!row.name || !row.short_name || !row.faculty_short_name) {
          errors.push(
            `Row ${lineNum}: Missing name, short_name, or faculty_short_name`,
          );
          continue;
        }
        ...
        const faculty = faculties.find(
          (f) =>
            f.short_name.toLowerCase() === row.faculty_short_name.toLowerCase(),
        );
        if (!faculty) {
          errors.push(
            `Row ${lineNum}: Faculty with short name "${row.faculty_short_name}" not found`,
          );
          continue;
        }
  ...
  async function loadData() {
    ...
    console.log(
      "DepartmentsPage: Loading data for university:",
    ...
    try {
      ...
      if (deptRes.error)
        console.error("Error fetching departments:", {
          message: deptRes.error.message,
          code: deptRes.error.code,
          details: deptRes.error.details,
          hint: deptRes.error.hint,
        });
      ...
      if (!staffRes.ok || sErr)
        throw new Error(sErr || "Failed to fetch staff via API");

      const hodData: Profile[] = (allProfiles || []).filter(
        (p: Profile & { role: string }) =>
          (p.role || "").toLowerCase().trim() === "hod",
      );
      ...
      hodData.forEach((h: Profile) => {
        hodMap[h.id] = h.full_name;
      });
      ...
      {importSuccess > 0 && importErrors.length === 0 && (
        ...
        >
          <p style={{ fontSize: "13px", color: "var(--success)" }}>
            ✓ {importSuccess} department{importSuccess !== 1 ? "s" : ""}{" "}
